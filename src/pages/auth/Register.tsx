        ...
        if (error) {
            ...
            if (data.user) {
                const { error: profileError } = await supabase
                    .from("profiles")
                    .insert([{ id: data.user.id, email: email, role: 'customer' }]);
                ...
                <div className="layout-content-container flex flex-col w-full max-w-[420px] bg-white rounded-2xl shadow-xl overflow-hidden border border-[#e7f3ec]">
                    <div className="flex flex-col items-center pt-12 pb-6 px-8 text-center">
                        ...
                        <p className="text-[#5c7a56] text-base font-normal leading-normal pt-2">
                            Join the realm of coffee rewards today.
                        </p>
                    </div>

                    <form onSubmit={handleRegister} className="px-8 pb-10 flex flex-col gap-5">
                        ...
                        <div className="flex flex-col items-center gap-1">
                            <p className="text-[#5c7a56] text-sm">Already have an account?</p>
                            <Link to="/auth/login" className="text-[#31512A] text-base font-bold hover:underline bg-transparent border-none p-0 cursor-pointer">
                                Sign In
                            </Link>
                        </div>
                    </form>
